---
name: qa-plan
description: Generate a step-by-step manual QA plan for freshly-built work by cross-referencing a GitHub issue against its diff, run a quick browser smoke first, and post the plan as a comment on the issue. Use as the final runtime gate after the static review skills (/tdd, /review, /codex:review) and before marking an issue ready-for-human or opening a PR — when the user says "qa this", "qa plan", "how do I test this", "make a test plan", "verify the new feature", "what should I click", or just finished implementing something and wants to check it by hand. For UI work it is also the visual gate — reach for it whenever new UI needs a screenshot, a responsive check, or a "does it actually look right in the browser" pass, not only when someone says "QA".
---
...
- **For visual items, assert the layout — not just the text.** A text check is blind to position: "the banner says 'scheduled for deletion'" passes even when that banner renders *underneath* the hero. Write each visual expectation in terms a screenshot can prove — *no overlap, no horizontal overflow, correct stacking order, aligned to the grid* — and pair it with the matrix frame that demonstrates it.
...
For UI-bearing changes, the plan additionally carries **layout-integrity checks** and an **embedded, GitHub-verified screenshot matrix** (a real desktop width plus the app's own breakpoint widths), each visual item paired with the frame that proves it.
