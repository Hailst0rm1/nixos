---
name: storm-research
description: Multi-perspective, citation-verified HTML research briefing on a contested topic. Use when the user says "storm research this" / "storm report on X" / wants a STORM briefing, or when a topic needs multiple expert viewpoints and fact-checked claims rather than a single answer. Runs a 4-phase pipeline: five expert lenses (Practitioner, Academic, Skeptic, Economist, Historian) -> contradiction map -> synthesized HTML report -> adversarial peer review + primary-source verification. Overkill for a simple factual lookup or primary-source fact-gathering (use /research for that).
...
Run the full pipeline end to end. This is heavier than a quick web lookup; that is the point.
...
This is what separates Storm Research from a normal report, and it is the phase most tempting to skip. Do not skip it. A report delivered without Phase 4 is not a Storm Research report — run it before delivering, and make the verification banner truthful.
