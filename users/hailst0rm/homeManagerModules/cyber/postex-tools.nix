{
  ...
  winpeasExe = pkgs.stdenv.mkDerivation {
    pname = "winpeasExe";
    version = "20260315-d7c1e6ce";
    src = pkgs.fetchurl {
      url = "https://github.com/peass-ng/PEASS-ng/releases/download/20260315-d7c1e6ce/winPEASany_ofs.exe";
      sha256 = "sha256-dAj9SxtqY0aQVI9LMwg5pQcfRiQqcQbCBR+xJ5NedTI=";
    };
    phases = ["installPhase"];
    installPhase = ''
      mkdir -p $out
  ...
  linpeas = pkgs.stdenv.mkDerivation {
    ...
    version = "20260315-d7c1e6ce";
    src = pkgs.fetchurl {
      url = "https://github.com/peass-ng/PEASS-ng/releases/download/20260315-d7c1e6ce/linpeas_fat.sh";
      sha256 = "sha256-OWRUBg7gHjeyss/hZVQRUig8y6gcLrqwWR8rqk7GB90=";
    };
    phases = ["installPhase"];
    installPhase = ''
      mkdir -p $out
  ...
  adPEAS = pkgs.stdenv.mkDerivation {
    ...
    version = "v2.2.0";
    src = pkgs.fetchurl {
      url = "https://github.com/61106960/adPEAS/raw/refs/tags/v2.2.0/adPEAS_obf.ps1";
      sha256 = "sha256-EoXMoJ/VF5PLI2rXQWUgNLQZlVTmjkV7F/rFP1+K48Q=";
    };
    phases = ["installPhase"];
    installPhase = ''
      mkdir -p $out
  ...
  config = lib.mkIf config.cyber.redTools.enable {
    home = {
      ...
      activation.copyTools = lib.hm.dag.entryAfter ["writeBoundary"] ''
        ...
        cp -f ${pkgs-unstable.bloodhound}/lib/BloodHound/resources/app/Collectors/SharpHound.exe "${config.home.homeDirectory}/cyber/postex-tools/SharpHound.exe"
