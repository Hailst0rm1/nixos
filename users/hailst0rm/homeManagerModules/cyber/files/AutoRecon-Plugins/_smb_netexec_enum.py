	...
	async def run(self, service):
		if self.get_global('username'):
			...
			if self.get_global('password'):
				...
				await service.execute('netexec smb {address} -u ' + username + ' -p ' + password + ' -M bitlocker', outfile='netexec_enum_bitlocker.txt')
				...
				await service.execute('netexec smb {address} -u ' + username + ' -p ' + password + ' -M keepass_discover', outfile='netexec_enum_keepass_discover.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -p ' + password + ' -M reg-winlogon', outfile='netexec_enum_reg_winlogon.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -p ' + password + ' -M recent_files', outfile='netexec_enum_recent_files.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -p ' + password + ' -M snipped', outfile='netexec_enum_snipped.txt')
			if self.get_global('nthash'):
				...
				await service.execute('netexec smb {address} -u ' + username + ' -H ' + nthash + ' -M bitlocker', outfile='netexec_enum_bitlocker.txt')
				...
				await service.execute('netexec smb {address} -u ' + username + ' -H ' + nthash + ' -M keepass_discover', outfile='netexec_enum_keepass_discover.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -H ' + nthash + ' -M reg-winlogon', outfile='netexec_enum_reg_winlogon.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -H ' + nthash + ' -M recent_files', outfile='netexec_enum_recent_files.txt')
				await service.execute('netexec smb {address} -u ' + username + ' -H ' + nthash + ' -M snipped', outfile='netexec_enum_snipped.txt')
		else:
