{
  ...
  config = lib.mkIf (config.importConfig.hyprland.enable && config.importConfig.hyprland.panel == "waybar") {
    ...
    # Filename kept as mocha.css so the @import above stays put; the
    # contents follow whichever theme is active.
    home.file.".config/waybar/mocha.css".text = config.paletteCss;
  };
}
